...
operator_t **init_oplist(char *ops)
{
    ...
    operators[2] = new_op("==", 1, 2, &equal);
    operators[3] = new_op(">=", 1, 2, &greater_or_equal);
    operators[4] = new_op("<=", 1, 2, &less_or_equal);
    operators[5] = new_op(">", 1, 1, &greater_than);
    operators[6] = new_op("<", 1, 1, &less_than);
